...
7. ~~What is the **minimum** legally required retention period for medical consultation records?~~ **ANSWERED 2026-08-29 by the product owner from MDC sources: a minimum does exist.** Deleting notes at completion breaches the record-keeping duty on registered practitioners under Act 857 and destroys evidence needed during the 3-year civil window for negligence. The design was changed accordingly — see decision D23 and `data-retention.md`. **Two follow-ups have since been answered — G7c (a sealed archive counts as complying) and G7g (the consultation summary has no mandated form). Five remain, tracked as G7a, G7b, G7d, G7e and G7f in `data-retention.md` §10:** the exact period within the stated 3–6 year range; whether paediatric records carry a longer rule; whether a sealed archive with no clinician access counts as complying when continuity of care is declined; the lawful basis and required form of patient notice; the access/erasure mechanism; and an acceptable backup window.

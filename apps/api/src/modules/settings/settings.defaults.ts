...
export const DEFAULT_SETTINGS: SettingDefinition[] = [
  ...
  },
  {
    ...
    value: 300,
    valueType: 'number',
    description: 'How long a pending payment stays valid before the consultation expires (spec §35).',
    category: 'consultation',
  },
  {
  ...
  { key: K.QUEUE_WEIGHT_LANGUAGE, value: 0.3, valueType: 'number', description: 'Queue weight: language proficiency.', category: 'queue', requiresConfirm: true },
  { key: K.QUEUE_WEIGHT_AVAILABILITY, value: 0.2, valueType: 'number', description: 'Queue weight: availability headroom.', category: 'queue', requiresConfirm: true },
  { key: K.QUEUE_WEIGHT_WORKLOAD, value: 0.2, valueType: 'number', description: 'Queue weight: workload balance across the shift.', category: 'queue', requiresConfirm: true },
  { key: K.QUEUE_WEIGHT_RESPONSE_TIME, value: 0.15, valueType: 'number', description: 'Queue weight: median offer-to-accept latency.', category: 'queue', requiresConfirm: true },
  { key: K.QUEUE_WEIGHT_RECENT_COUNT, value: 0.1, valueType: 'number', description: 'Queue weight: consultations completed in the trailing 24 hours.', category: 'queue', requiresConfirm: true },
  { key: K.QUEUE_WEIGHT_QUALITY, value: 0.05, valueType: 'number', description: 'Queue weight: internal quality score.', category: 'queue', requiresConfirm: true },

  // --- Quality score (never visible to doctors — spec §52) ----------------
  { key: K.QUALITY_WEIGHT_RATING, value: 0.3, valueType: 'number', description: 'Quality weight: mean patient rating.', category: 'quality', requiresConfirm: true },
  { key: K.QUALITY_WEIGHT_COMPLAINTS, value: 0.2, valueType: 'number', description: 'Quality weight: complaint rate (negative).', category: 'quality', requiresConfirm: true },
  { key: K.QUALITY_WEIGHT_RESPONSE_TIME, value: 0.15, valueType: 'number', description: 'Quality weight: median response time.', category: 'quality', requiresConfirm: true },
  { key: K.QUALITY_WEIGHT_MISSED, value: 0.15, valueType: 'number', description: 'Quality weight: missed-response rate (negative).', category: 'quality', requiresConfirm: true },
  { key: K.QUALITY_WEIGHT_COMPLETION, value: 0.1, valueType: 'number', description: 'Quality weight: consultation completion rate.', category: 'quality', requiresConfirm: true },
  { key: K.QUALITY_WEIGHT_AUDIT, value: 0.05, valueType: 'number', description: 'Quality weight: clinical and administrative audit outcomes.', category: 'quality', requiresConfirm: true },
  { key: K.QUALITY_WEIGHT_RX_ISSUES, value: 0.05, valueType: 'number', description: 'Quality weight: prescription issues (negative).', category: 'quality', requiresConfirm: true },
  ...
  },
  {
    ...
    value: 50000,
    valueType: 'number',
    description: 'Six-month doctor membership fee in minor units. Seeded placeholder — set before go-live.',
    category: 'workforce',
    requiresConfirm: true,
  },
  { key: K.DOCTOR_MEMBERSHIP_MONTHS, value: 6, valueType: 'number', description: 'Doctor membership period in months.', category: 'workforce' },
  { key: K.DOCTOR_MEMBERSHIP_GRACE_DAYS, value: 7, valueType: 'number', description: 'Grace period after membership expiry before Active → Suspended (spec §27).', category: 'workforce' },
  { key: K.DOCTOR_MIN_YEARS_EXPERIENCE, value: 3, valueType: 'number', description: 'Minimum post-qualification clinical experience in years (spec §21).', category: 'workforce' },
  { key: K.DOCTOR_LICENCE_WARNING_DAYS, value: 60, valueType: 'number', description: 'Days before MDC licence expiry at which the system flags the doctor (spec §22).', category: 'workforce' },
